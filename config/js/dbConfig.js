...
fs.ensureFile(window.localStorage.dbPath).then(()=>{
  ...
  try {
    ...
    db.serialize(function() {
        ...
        db.exec('CREATE TABLE IF NOT EXISTS ' + commonNames.services + '("id"  INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,"Created_Date" datetime, "Stock" INT, "Sale_Route" INT,"Service_Agent" INT,"Status" INT,"Other" TEXT);')
        db.exec('CREATE TABLE IF NOT EXISTS ' + commonNames.serviceStatus + '("id"  INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,"Created_Date" datetime, "Serv_Req" INT, "Status" INT);')
